...
pause()
r = getDeformedTyre(tyre_s , road_s, 1 ,1 , constants_s);
plot_range = mod([tyre_s.contact_boundary_inds(1 , 2):tyre_s.contact_boundary_inds(1 , 2) + 100] , 360) + 1;
...
r = getDeformedTyre(tyre_s , road_s, 1 ,-1 , constants_s);
plot_range = mod([tyre_s.contact_boundary_inds(1 , 1):-1:tyre_s.contact_boundary_inds(1 , 1) - 100] , 360);
plot(tyre_s.x_centre + r(plot_range).*cos(tyre_s.theta(plot_range)) , tyre_s.y_centre+ r(plot_range).*sin(tyre_s.theta(plot_range)),LineWidth=3);
...
function deformed_tyre = getDeformedTyre(tyre, road, contact_ind, direction, constants)
    ...
    initial_penetration = initial_point_polar(1) - tyre.free_radius;
    initial_theta =  pi/2 + initial_point_polar(2) -(atan(road.gradient(current_ind_road)));
    deformed_tyre(current_ind_tyre) = tyre.free_radius + initial_penetration;
    current_ind_tyre = current_ind_tyre + 1;
    while(exp(-constants.beta*current_theta) > 0.01)
        deformed_tyre(mod(current_ind_tyre -1 , num_nodes) + 1) = tyre.free_radius + ...
         ...
         (tan(initial_theta)/constants.beta + initial_penetration) *sin(constants.beta*current_theta));
